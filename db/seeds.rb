...
Page.create!(user_id:1,
             title:"Exponential: Order and Chaos in an Age of Accelerating Technology",
             slug:"exponential",
             author: "Azeem Azhar",
             publisher: "Cornerstone Digital",
             publisher_url: "https://www.penguin.co.uk/books/441674/exponential-by-azeem-azhar/9781847942920",
             published: true,
             tags_string: "Non-fiction-story",
             summary: "テクノロジーの進歩が指数関数的な速度で進んでいることを様々な例をあげて指摘している本。その取り上げられる例が自分が知らなかった分野や詳しくない分野を含んでいるため、そんな変化が訪れているのかという好奇心を刺激して面白く読める。作者はその変化に人々が対応しなければいけないことは指摘するが、どう対応していくかまでは述べていない。そこから先を考えてみないかとあなたに問いかける本なのである。",
             content: "この本はよく大型書店の洋書売り場で見かけた本だったが、翻訳は出てないようである。私は英語の勉強をかねて読み始めたが、英語中級者でも読みやすい分かりやすい英語で書かれている本だった。つまり、今の時代は指数級数的にテクノロジーが進化しており、それに法律や私達の感覚が追いついてないということを言いたいようだが、結論はぼんやりとしか述べられておらず、むしろ現状分析に徹して、自分で考えてみてという本である。ムーアの法則といえば、何の科学的根拠もないにも関わらず、この時代の産業の進歩を証明し続けている法則がある。ムーアーの法則によれば半導体の集積率は3年で倍になるという、これは面積において元の面積の2乗になることを意味し半導体の集積率は指数級数的に増加し続けていることになる。もちろんこれは半導体に限った話ではないので、著者は他の産業分野での指数関数的増加についても言及している。例えばリチウムイオン電池の小型化などがその例である。この本には言及されていないが、AIの計算には元々は3Dグラフィック向けて当初は開発されたGPUというチップが応用されている。NVIDIAという企業は3Dグラフィック用のチップで成長したが、いち早くAI計算向けにこの技術を応用したチップを大量生産して供給することで、今日のChat GPTのようなAIが当たり前に使える時代を可能にした。AIが急速に進化して私達が当たり前のように毎日使うものになるには、GPUにおける集積率の指数急素敵な増加が関わっているのである。いや私達のほうがAIをいかに取り込むか、つまりこの指数関数的なテクノロジーの進化を前にして常に変化していく生活に適応していけるかが問われているのは、ChatGPT関連の本の出版ブームに見てとれる。テクノロジーの指数関数的進歩が可能にしたテクノロジーの進歩を自分の興味のある分野だけではなく、俯瞰して見せてくれるのがこの本の面白いところだろう。例えば太陽電池は指数関数的に面積に対する発電効率が増しており、間違いなく成長産業の一つである。太陽電池の開発競争において日本政府は誘導的な普及政策（例えば自然エネルギーの価格を低く抑えるために価格設定に国が基準を設け、違反すれば電力会社から罰金をとり、自然エネルギーの比率を上げた会社に補助金を出すなどが考えられる）をとらなかったため、日本の太陽電池を開発していたメーカーは世界の競争についていけなくなってしまった。古い技術が新しい技術によって駆逐されることが、これからも指数関数的なテクノロジーの発展が続く限りおこるだろう。私達は指数関数的なテクノロジーの進歩がおこる時代では、今日やっていたゲームのルールが全く違う新しいルールをもつ新しいゲームに変わっているような変化に対応していかなければいけない。これが私がこの本で得た率直な感想である。")
